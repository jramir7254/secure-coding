...
export default function GameCard({ game, children, className, ...props }: { game: GameSchema, children?: React.ReactNode } & React.ComponentProps<'div'>) {
    ...
    const { isActive, startedAt, endedAt, id, maxTeams, timeLimit, teamsPlayed } = game

    const isFull = (currentTeams?.length || 0) === maxTeams

    return (
        <DevCard className={cn(className, !isActive ? 'flex flex-row items-center h-15 py-2' : 'flex-col py-5', 'px-5 relative overflow-hidden font-nunit flex gap-3')} {...props}>
            <h3 className='font-bold text-lg'>Game #{id}</h3>
            <Separator className='border h-full' orientation={isActive ? 'horizontal' : 'vertical'} />
            ...
            <p hidden={!isActive}><strong>Time Limit:</strong> <span className='text-muted-foreground'>{msToMinutes(timeLimit)} minutes</span></p>

            <div hidden={!isActive} className='flex gap-10'>
